...
\newcommand{\pltlang}{\pltlang\.} % allows us to just reference the language name with the ``\pltlang'' cmd, so we can change the name later w/o too much hassle
...
\pretitle{%
  \begin{center}
  \LARGE
  \includegraphics[width=7cm,height=7cm,keepaspectratio]{dots.png}\\[\bigskipamount]
}
...
\pltlang\ programs must be compiled on systems that can run shell scripts and executables from the comma 

To compile a \pltlang\ file directly to an executable, use \code{gdc}. Assuming \code{gdc} is in the current directory:
...
%have to reorganize headings of LRM.... 
...
\subsubsubsection{Primitive Types}
...
\subsubsubsection{Node and Graph Objects}
...
\subsubsubsection{Number and String Operators}
...
\subsubsubsection{Node Operators}
...
\subsubsubsection{Graph Operators}
...
\subsubsubsection{Subscript Notation}
...
\subsubsubsection{Operator Precendence}
...
\subsubsubsection{Member Access}
...
\subsubsubsection{Control Flow Statements}
...
\subsubsubsection{Loop Statements}
...
\subsubsubsection{Other Statements}
...
\subsubsubsection{Function Declaration and Definition}
...
\subsubsubsection{Return Statements}
...
\subsubsubsection{Parameter List}
...
\subsubsubsection{Calling Functions}
...
\subsubsubsection{Variable Length Parameter Lists}
...
\subsubsubsection{Built-in Functions}
...
\subsection{Program Structure and Scope}
...
\subsubsubsection{Expression Statements}
...
\pltlang\ was developed on UNIX operating systems (various Mac OSes or Ubuntu 14.04) with editors such as Sublime Text or Vim. 
...
% basically copy and paste code file into lstlisting
\section{Appendix}
