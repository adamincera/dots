...
Operators are operations that are performed on operands which can be functions, variables, or data types. 
\textbf{Numerical operators} are operators that include arithmetic and comparative operators.

\emph{Arithmetic Operators} \\
Arithmetic Operators function exactly like C with primitive data types such as literals,  variables and functions. d.o.t.s support the addition, subtraction, multiplication, division and modular divison operators. Similar to C, integer division truncates the resulting value \code {5 / 3 = 1}. Modular division returns the remainder of a given division ie \code {5 \% 3 = 2} 

\emph{Comparative Operators} \\
These opeartors relate two operands to each other. Some sample comparisons include: 
...
all operators function as they do in C as long as the operands are primitive data types, variables or functions. The result of a comparison operation is a boolean. 
...
Logical operators evaluate the logical truth of two operands. This set of functions operates similarly to the logical operators.

\code{==} and \code{!=} are the only logical operators which function on graph operands. These operators are evaluated by recursively evaluating the Node \code{==} and \code{!=} logical comparison operators.

== signifies that two graphs have the same vertices and edges.

!= signifies that two graphs do not have the same vertices and edges.
\subsection{Subscript Notation}

\subsubsection{Subscript}
\emph{dict subscripts} specify elements within a \code{dict} at a given index. The notation is “D[i]” where brackets denote the key in the dictionary you want to index.
\emph{list subscripts} specify elements within a \code{list} at a given index. The notation is “L[i]” where brackets denote the position in list L. 


\subsection{Operator Precendence}
follows traditional mathematical order of operations: parenthesis, exponent, multiply, divide, addition, subtraction
...
print x.in; # prints [r: 0.39]

print x.in[r] # dict subscript notation; prints 0.39
